...
def CreateHT():  # 构造哈夫曼树
    ...
    for i in range(n0, 2 * n0 - 1):  # i从n0到2n0-2循环做n0-1次合并操作
        ...
        ht[i].lchild = p1[1]  # 此处添加一条语句，将p1作为双亲ht[i]的左孩子
        ht[p1[1]].flag=True
        ht[p2[1]].parent = i  # 设置p2的双亲为ht[i]
        ht[i].rchild = p2[1]  # 此处添加一条语句，将p2作为双亲ht[i]的右孩子
        ht[p2[1]].flag = False
...
if __name__ == '__main__':
    n0 = 8  # 编码的字符个数
    D = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']  # 字符列表
    W = [5, 25, 3, 6, 10, 11, 36, 4]  # 权值列表
    print()
    print("  (1)建立哈夫曼树")
    CreateHT()
    print("  (2)输出哈夫曼树")
